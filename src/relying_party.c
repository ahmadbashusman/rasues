...
static unsigned int port = COAP_DEFAULT_PORT;
#define CBOR_ENCODER_BUFFER_LENGTH 20480
...
int main(int argc, char** argv) {
    ...
    charra_log_level_from_str((const char*)getenv("LOG_LEVEL_CHARRA"), &charra_log_level);
    charra_coap_log_level_from_str((const char*)getenv("LOG_LEVEL_COAP"), &coap_log_level);
    ...
    cli_config cli_config = {
        ...
        .common_config = {
            ...
            .dtls_rpk_verify_peer_public_key = &dtls_rpk_verify_peer_public_key,
        },
        .rparty_config = {
    ...
    charra_log_debug("[" LOG_NAME "]     DTLS-PSK enabled: %s", (use_dtls_psk == true) ? "true" : "false");
    if (use_dtls_psk) {
        charra_log_debug("[" LOG_NAME "]         Pre-shared key: '%s'", dtls_psk_key);
        charra_log_debug("[" LOG_NAME "]         Hint: '%s'", dtls_psk_hint);
    }
    charra_log_debug("[" LOG_NAME "]     DTLS-RPK enabled: %s", (use_dtls_rpk == true) ? "true" : "false");
    if (use_dtls_rpk) {
        charra_log_debug("[" LOG_NAME "]         Private key path: '%s'", dtls_rpk_private_key_path);
        charra_log_debug("[" LOG_NAME "]         Public key path: '%s'", dtls_rpk_public_key_path);
        charra_log_debug("[" LOG_NAME "]         Peers' public key path: '%s'", dtls_rpk_peer_public_key_path);
    }

    /* set variables here such that they are valid in case of a 'goto error' */
    coap_context_t* coap_context = NULL;
    coap_endpoint_t* coap_endpoint = NULL;

    if (use_dtls_psk && use_dtls_rpk) {
        charra_log_error("[" LOG_NAME "] Configuration enables both DTSL with PSK and DTSL with PKI. Aborting!");
        goto error;
    }
    ...
    if ((use_dtls_psk || use_dtls_psk) && !coap_dtls_is_supported()) {
        charra_log_error("[" LOG_NAME "] CoAP does not support DTLS but the configuration enables DTLS. Aborting!");
        goto error;
    }
    ...
    if (use_dtls_psk) {
        charra_log_info("[" LOG_NAME "] Creating CoAP server endpoint using DTLS-PSK.");
        if (!coap_context_set_psk(coap_context, dtls_psk_hint, (uint8_t*)dtls_psk_key, strlen(dtls_psk_key))) {
            charra_log_error("[" LOG_NAME "] Error while configuring CoAP to use DTLS-PSK.");
            goto error;
        }

        if ((coap_endpoint = charra_coap_new_endpoint(coap_context, LISTEN_ADDRESS, port, COAP_PROTO_DTLS)) == NULL) {
            charra_log_error("[" LOG_NAME "] Cannot create CoAP server endpoint based on DTLS-PSK.\n");
            goto error;
        }
    } else if (use_dtls_rpk) {
        charra_log_info("[" LOG_NAME "] Creating CoAP server endpoint using DTLS-RPK.");
        ...
        if (rc != CHARRA_RC_SUCCESS) {
            charra_log_error("[" LOG_NAME "] Error while setting up DTLS-RPK structure.");
            goto error;
        }

        if (!coap_context_set_pki(coap_context, &dtls_pki)) {
            charra_log_error("[" LOG_NAME "] Error while configuring CoAP to use DTLS-RPK.");
            goto error;
        }

        if ((coap_endpoint = charra_coap_new_endpoint(coap_context, LISTEN_ADDRESS, port, COAP_PROTO_DTLS)) == NULL) {
            charra_log_error("[" LOG_NAME "] Cannot create CoAP server endpoint based on DTLS-RPK.\n");
            goto error;
        }
    } else {    
        charra_log_info("[" LOG_NAME "] Creating CoAP server endpoint using UDP.");
        if ((coap_endpoint = charra_coap_new_endpoint(coap_context, LISTEN_ADDRESS, port, COAP_PROTO_UDP)) == NULL) {
            charra_log_error("[" LOG_NAME "] Cannot create CoAP server endpoint based on UDP.\n");
            goto error;
        }
    }
    ...
    charra_coap_add_resource(coap_context, COAP_REQUEST_FETCH, "attRes", coap_attest_result_handler);
    ...
    while (!quit) {
        ...
        if (coap_io_process(coap_context, COAP_IO_WAIT) == -1) {
            charra_log_error("[" LOG_NAME "] Error during CoAP I/O processing.");
            goto error;
        } 
    }
...
static void coap_attest_result_handler(struct coap_context_t* context CHARRA_UNUSED,
    ...
    charra_log_info("[" LOG_NAME "] Resource '%s': Received message.", "attRes");
    ...
    if ((coap_r = coap_get_data_large(in, &data_len, &data, &data_offset, &data_total_len)) == 0) {
        ...
        charra_log_info("[" LOG_NAME "] Received data of total length %zu.", data_total_len);
    }

    /* unmarshal data */
    charra_log_info("[" LOG_NAME "] Parsing received CBOR data.");
    msg_attestation_appraise_result_dto att_result = {0};
    if ((charra_r = charra_unmarshal_attestation_passport(data_len, data, &att_result)) != CHARRA_RC_SUCCESS) {
        charra_log_error("[" LOG_NAME "] Could not parse CBOR data.");
    } else {
        charra_log_info("[" LOG_NAME "] Attestation Result Unmarshalled");
    }
    ...
    if ((charra_verify_att_result(verifier_public_key_path, att_result.attestation_result_data, 
        ...
        charra_log_error("[" LOG_NAME "] error verifying signature attestation result.");
    } else {
